...
function setupNavigation() {
  ...
  const experimentURLs = {
    ...
    'BJT Amplifier': 'bjt_amplifier.html',
    'BJT Common Emitter Amplifier': 'bjt_amplifier.html',
    'Inverting Amplifier': 'inverting_amplifier.html',
    'Common Source Amplifier': 'mosfet_simulator.html',
    'CMOS Inverter': 'coming_soon.html?circuit=cmos-inverter',
    'Active Filter Design': 'coming_soon.html?circuit=active-filter',
    'Current Mirror Circuit': 'coming_soon.html?circuit=current-mirror',
    'Differential Amplifier': 'coming_soon.html?circuit=differential-amplifier',
    'Sequential Logic Circuits': 'coming_soon.html?circuit=sequential-logic',
    'Phase-Locked Loop': 'coming_soon.html?circuit=pll',
    'JFET Characteristics': 'coming_soon.html?circuit=jfet-characteristics',
    'Schmitt Trigger Design': 'coming_soon.html?circuit=schmitt-trigger',
  ...
  document.querySelectorAll('.experiment-link, .action-btn').forEach(element => {
    element.addEventListener('click', function(e) {
      ...
      if (this.textContent.trim() === 'Review' || 
        ...
        // If the element already has an href attribute, we don't need to handle the click
        ...
        if (parent) {
          ...
          if (experimentURLs[title]) {
            // For activity items, we need to check if they have a data-href attribute
            if (parent.classList.contains('activity-item') && parent.getAttribute('data-href')) {
              window.location.href = parent.getAttribute('data-href');
            } else {
  ...
  document.querySelectorAll('.main-nav a').forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      if (href === 'index.html' || href === 'mosfet_simulator.html') {
        return; // Let the default behavior handle the navigation
      }
        ...
        const newExperiments = [
          {
            ...
            date: 'Completed on Mar 10, 2025',
            image: 'images/WhatsApp Image 2025-04-10 at 14.45.20 (2).jpeg',
            grade: 'B',
            score: '85%'
          },
          {
            title: 'Voltage Regulator',
            date: 'Completed on Mar 5, 2025',
            image: 'images/WhatsApp Image 2025-04-10 at 14.45.22 (1).jpeg',
            grade: 'A',
            score: '92%'
          },
          {
            ...
            image: 'images/WhatsApp Image 2025-04-10 at 14.45.22 (2).jpeg',
            grade: 'A-',
            score: '90%'
          }
        ];
...
function addToPendingExperiments(title) {
  // Use Active Filter Design image for new experiments
  let imageUrl = 'images/WhatsApp Image 2025-04-10 at 14.45.22 (2).jpeg';
